...
export default function AdminDashboardScreen() {
  ...
  const [activeTab, setActiveTab] = useState('requests'); // requests | users | tutors | resources | videos | ai-insights | faq-automation
    ...
    <SafeAreaView style={styles.container}>
      ...
      {isAdmin && (
        <>
          ...
          {activeTab === 'users' && (
            <FlatList
              data={users.filter(u => !u.isAdmin && !u.isTutor)}
              keyExtractor={(i)=>i.id}
              ...
              renderItem={({item}) => (
                <View style={styles.resourceRow}>
                  ...
                  <View style={{ flexDirection:'row', alignItems:'center' }}>
                    ...
                    <TouchableOpacity onPress={() => { try { if (item.videoUrl) Linking.openURL(item.videoUrl); } catch {} }} style={{ marginRight: 10 }}>
                      <Ionicons name="open-outline" size={20} color="#007AFF" />
                    </TouchableOpacity>
                    <TouchableOpacity onPress={() => setEditVideo(item)} style={{ marginRight: 10 }}>
              ...
              {aiInsights.insights.length === 0 && !aiInsights.loading && (
                <View style={styles.emptyState}>
                  ...
                  <View style={{ marginTop: 16, padding: 12, backgroundColor: '#f8f9fa', borderRadius: 8 }}>
                    ...
                    <Text style={{ fontSize: 10, color: '#666' }}>Users: {users?.length || 0}</Text>
                    <Text style={{ fontSize: 10, color: '#666' }}>Resources: {resources?.length || 0}</Text>
                    <Text style={{ fontSize: 10, color: '#666' }}>Videos: {videos?.length || 0}</Text>
                    <Text style={{ fontSize: 10, color: '#666' }}>Loading: {aiInsights.loading ? 'Yes' : 'No'}</Text>
                    <Text style={{ fontSize: 10, color: '#666' }}>Insights: {aiInsights.insights?.length || 0}</Text>
                  </View>
                </View>
              )}
            </ScrollView>
                        ...
                        {question.aiResponse && (
                          <View style={styles.aiResponseSection}>
                            <View style={styles.aiResponseHeader}>
                              <Ionicons name="robot-outline" size={16} color="#6366f1" />
...
const styles = StyleSheet.create({
  ...
  list: { padding: 12 },
  tabs: { flexDirection:'row', backgroundColor:'#fff', margin:12, borderRadius:12, borderWidth:1, borderColor:'#eee' },
  ...
  userRow: { flexDirection:'row', alignItems:'center', backgroundColor:'#fff', borderRadius:12, padding:12, marginBottom:10 },
  userEmail: { fontSize:14, fontWeight:'600', color:'#111' },
  ...
  resourceRow: { flexDirection:'row', alignItems:'center', backgroundColor:'#fff', borderRadius:12, padding:12, marginBottom:10 },
  resourceTitle: { fontSize:14, fontWeight:'600', color:'#111' },
  resourceMeta: { fontSize:12, color:'#666' },
